...
func TestKite_readConfig(t *testing.T) {
	app := App{}
...
func TestKite_isPortAvailable(t *testing.T) {
...
func TestPingKite(t *testing.T) {
	tests := []struct {
		name        string
		input       bool
		expectedURL string
	}{
		{"Ping Start Server", true, kiteHost + startServerPing},
		{"Ping Shut Server", false, kiteHost + shutServerPing},
	}
...
func TestKite_ServerRoutes(t *testing.T) {
	_ = testutil.NewServerConfigs(t)
...
func TestKite_ServerRun(t *testing.T) {
	configs := testutil.NewServerConfigs(t)
...
func Test_EnableBasicAuthWithValidator(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			validateFunc := func(_ *infra.Container, username string, password string) bool {
...
func Test_initTracer(t *testing.T) {
	...
	mockConfig7 := createMockConfig("kite", "", "")
	...
	tests := []struct {
		...
		{"kite exporter with default url", mockConfig7, "Exporting traces to Kite at https://tracer-api.github.com/sllt/kite/api/spans"},
	}
...
func TestUseMiddlewareWithContainer(t *testing.T) {
	...
	mockContainer := infra.NewContainer(config.NewMockConfig(nil))
	...
	middleware := func(c *infra.Container, handler http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func Test_APIKeyAuthMiddleware(t *testing.T) {
	...
	validateFunc := func(_ *infra.Container, apiKey string) bool {
...
func Test_AddCronJob_Fail(t *testing.T) {
	a := App{container: &infra.Container{}}
...
func setupTestEnvironment(t *testing.T) (host string, htmlContent []byte) {
	...
	app.AddStaticFiles("kiteTest", "testdir")
...
func TestStaticHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc: "check file content index.html in custom dir", method: http.MethodGet, path: "/" + "kiteTest" + "/" + indexHTML,
			statusCode: http.StatusOK, expectedBodyLength: len(htmlContent),
			expectedResponseHeaderType: "text/html; charset=utf-8", expectedBody: string(htmlContent),
		},
		{
			desc: "check public endpoint in custom dir", method: http.MethodGet, path: "/" + "kiteTest",
...
func TestStaticHandlerInvalidFilePath(t *testing.T) {
	...
	logs := testutil.StderrOutputForFunc(func() {
		...
		app.AddStaticFiles("kiteTest", ".//,.!@#$%^&")
	})

	assert.Contains(t, logs, "no such file or directory")
	assert.Contains(t, logs, "error in registering '/kiteTest' static endpoint")
}
