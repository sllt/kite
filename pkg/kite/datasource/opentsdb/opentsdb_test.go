	...
	mockHTTP.EXPECT().
		...
		Return(&http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`[{"metric":"net","timestamp":1728836485000,"value":` +
				`"19.499737232159088","tags":{"demo-name":"opentsdb-test","host":"kite-host","try-name":` +
				`"kite-sample"},"tsuid":"000003000001000001000002000007000003000008"},{"metric":"disk",` +
				`"timestamp":1728836485000,"value":"98.53097270356102","tags":{"demo-name":"opentsdb-test",` +
				`"host":"kite-host","try-name":"kite-sample"},"tsuid":"000002000001000001000002000007000003000008"}` +
				`,{"metric":"cpu","timestamp":1728836485000,"value":"49.47446557839882","tags":{"demo-name":"opentsdb` +
				`-test","host":"kite-host","try-name":"kite-sample"},"tsuid":"000001000001000001000002000007000003000008"}` +
				`,{"metric":"mem","timestamp":1728836485000,"value":"28.62340008609452","tags":{"demo-name":"opentsdb-test",` +
				`"host":"kite-host","try-name":"kite-sample"},"tsuid":"000004000001000001000002000007000003000008"}]`)),
		}, nil).Times(1)
...
func TestUpdateAnnotationSuccess(t *testing.T) {
	...
	anno := Annotation{
		StartTime:   addedST,
		TSUID:       addedTsuid,
		Description: "kitef test annotation",
		Notes:       "These would be details about the event, the description is just a summary",
		Custom:      custom,
	}

	expectedResponse := `{"tsuid":"000001000001000002","description":"kitef test annotation","notes":` +
		`"These would be details about the event, the description is just a summary","custom":{"host":` +
		`"kite-host","owner":"kite"},"startTime":` + fmt.Sprintf("%d", addedST) + `,"endTime":0}`
...
func TestQueryAnnotationSuccess(t *testing.T) {
	...
	anno := Annotation{
		StartTime:   addedST,
		TSUID:       addedTsuid,
		Description: "kite test annotation",
		Notes:       "These would be details about the event, the description is just a summary",
		Custom:      custom,
	}

	mockResponse := `{"tsuid":"000001000001000002","description":"kite test annotation","notes":"These` +
		` would be details about the event, the description is just a summary","custom":{"host"` +
		`:"kite-host","owner":"kite"},"startTime":1728841614,"endTime":0}`
...
func TestDeleteAnnotationSuccess(t *testing.T) {
	...
	anno := Annotation{
		StartTime:   addedST,
		TSUID:       addedTsuid,
		Description: "kite-host test annotation",
		Notes:       "These would be details about the event, the description is just a summary",
		Custom:      custom,
	}
	...
	mockHTTP.EXPECT().
		...
		Return(&http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{
				...
				"description": "kite-host test annotation",
				"notes": "These would be details about the event, the description is just a summary",
				"custom": {"host": "kite-host", "owner": "kite"},
