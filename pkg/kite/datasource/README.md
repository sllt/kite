...
Kite provides following features to ensure robust and observable interactions with various data sources:
...
Kite attempts to re-establish connections if lost during application runtime.
...
Kite supports level-based logging with the PrettyPrint interface.
...
Kite supports tracing for all the datasources, for example for SQL it traces the request using https://github.com/XSAM/otelsql.
...
Kite offers built-in support for popular datasources like SQL (MySQL, PostgreSQL, SQLite), Redis, and Pub/Sub (MQTT, Kafka, Google as a backend). Including additional functionalities within the core Kite binary would increase the application size unnecessarily.

Therefore, Kite utilizes a pluggable approach for new datasources by separating implementation in the following way:
...
- Method Registration:
   ...
   Create a method in kite.go (similar to the existing one) that accepts the newly defined interface.


- Separate Repository:

   Develop a separate repository to implement the interface for the new datasource.
   This approach ensures that the new datasource dependency is only loaded when utilized, minimizing binary size for Kite applications. It also empowers users to create custom implementations beyond the defaults provided by Kite.
