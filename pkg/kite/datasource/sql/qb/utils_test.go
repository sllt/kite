...
type symbolOnlyAggregate string

func (s symbolOnlyAggregate) Symbol() string {
	return string(s)
}

func TestAggregateQuery_WithSymbolBuilder(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)

	mock.ExpectQuery("SELECT count\\(\\*\\) FROM tb1").WillReturnRows(sqlmock.NewRows([]string{"count(*)"}).AddRow(5))

	result, err := AggregateQuery(context.Background(), db, "tb1", nil, symbolOnlyAggregate("count(*)"))
	require.NoError(t, err)
	assert.Equal(t, int64(5), result.Int64())
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestAggregateQuery_InvalidBuilder(t *testing.T) {
	db, _, err := sqlmock.New()
	require.NoError(t, err)

	_, err = AggregateQuery(context.Background(), db, "tb1", nil, 123)
	assert.ErrorIs(t, err, errInvalidAggregateBuilder)
}
...
func TestOmitEmpty_DoesNotMutateInput(t *testing.T) {
	src := map[string]interface{}{
		"a": "",
		"b": "value",
	}

	out := OmitEmpty(src, []string{"a"})

	assert.Equal(t, map[string]interface{}{"a": "", "b": "value"}, src)
	assert.Equal(t, map[string]interface{}{"b": "value"}, out)
}
...
func TestJsonArrayAppend(t *testing.T) {
	...
	var testData = []struct {
		...
		},
		{
			...
			out: outStruct{
				cond: "UPDATE tb SET my_json=JSON_ARRAY_APPEND(my_json,?,?,?,JSON_ARRAY(?,?,?)) WHERE (id IN (?,?,?))",
				vals: []interface{}{"$", 0, "$[0]", 7, 8, 9, 4, 5, 6},
				err:  nil,
			},
		},
	}
...
func TestJsonSet(t *testing.T) {
	...
	var testData = []struct {
	...
	}{
		{
			...
			out: outStruct{
				cond: "UPDATE xx SET my_json=JSON_SET(my_json,?,?,?,JSON_ARRAY(?,?,?)),name=? WHERE (id=?)",
				vals: []interface{}{"$.a", 0, "$.friend_id", 7, 8, 9, "newName", 8},
				err:  nil,
			},
		},
		{
			...
			out: outStruct{
				cond: "UPDATE xx SET my_json=JSON_SET(my_json,?,JSON_ARRAY(?,?,?)) WHERE (id IN (?,?,?))",
...
func TestJsonContains_UnsupportedTypeReturnsError(t *testing.T) {
	_, _, err := BuildSelect("xx", map[string]interface{}{
		"_custom_0": JsonContains("my_json->'$'", struct{ Name string }{Name: "kite"}),
	}, nil)

	assert.ErrorIs(t, err, errUnsupportedJSONType)
}

func TestJsonSet_InvalidPathReturnsError(t *testing.T) {
	_, _, err := BuildUpdate("xx", map[string]interface{}{"id": 1}, map[string]interface{}{
		"_custom_0": JsonSet("my_json", 1, "v"),
	})
	assert.ErrorIs(t, err, errInvalidJSONPathType)

	_, _, err = BuildUpdate("xx", map[string]interface{}{"id": 1}, map[string]interface{}{
		"_custom_0": JsonSet("my_json", "   ", "v"),
	})
	assert.ErrorIs(t, err, errInvalidJSONPathValue)
}

func TestJsonRemove_InvalidPathReturnsError(t *testing.T) {
	_, _, err := BuildUpdate("xx", map[string]interface{}{"id": 1}, map[string]interface{}{
...
func TestJsonRemove(t *testing.T) {
	...
	var testData = []struct {
	...
	}{
		{
			...
			out: outStruct{
				cond: "UPDATE xx SET my_json=JSON_REMOVE(my_json,?,?),name=? WHERE (JSON_CONTAINS(my_json->'$.list[last]','null'))",
				vals: []interface{}{"$.unused", "$.list[last]", "newName"},
				err:  nil,
			},
		},
		{
...
func TestJsonArrayInsert(t *testing.T) {
	...
	var testData = []struct {
	...
	}{
		{
			...
			out: outStruct{
				cond: "UPDATE xx SET my_json=JSON_ARRAY_INSERT(my_json,?,?,?,?),a=?,name=? WHERE ((((? MEMBER OF(my_json->'$[0]'))) OR ((? MEMBER OF(my_json->'$[0]')))))",
				vals: []interface{}{"$[0]", "first", "$[1]", 2, 18, "newName", 1, 7},
				err:  nil,
			},
		},
		{
			...
			out: outStruct{
				cond: "UPDATE xx SET my_json=JSON_ARRAY_INSERT(my_json,?,?,?,JSON_ARRAY(?,?,?)) WHERE (id IN (?,?,?))",
				vals: []interface{}{"$[0]", 0, "$[0]", 7, 8, 9, 4, 5, 6},
				err:  nil,
			},
		},
	}
