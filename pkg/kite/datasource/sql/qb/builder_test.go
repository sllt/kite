	...
	var data = []struct {
		...
		},
		{
			...
			out: outStruct{
				cond: "",
				vals: nil,
				err:  errLimitOffsetNotSupported,
			},
		},
		{
			in: inStruct{
...
func TestBuildBetween_InvalidLengthReturnsError(t *testing.T) {
	cond, vals, err := BuildSelect("tb", map[string]interface{}{
		"age between": []int{10},
	}, nil)
...
func TestBuildSelect_InvalidGroupByReturnsError(t *testing.T) {
	cond, vals, err := BuildSelect("tb", map[string]interface{}{
		"_groupby": "department;drop table users",
	}, nil)

	assert.Equal(t, "", cond)
	assert.Nil(t, vals)
	assert.ErrorIs(t, err, errGroupByValueInvalid)
}
