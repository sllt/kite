...
func Test_ClickHouseGetLastMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		mockClickhouse.EXPECT().Select(gomock.Any(), gomock.Any(), getLastChKiteMigration).Return(tc.err)
...
func Test_ClickHouseCommitMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		mockClickhouse.EXPECT().Exec(gomock.Any(), insertChKiteMigrationRow, td.MigrationNumber,
