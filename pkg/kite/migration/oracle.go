...
func (om oracleMigrator) getLastMigration(c *infra.Container) (int64, error) {
	...
	err := om.Oracle.Select(context.Background(), &results, getLastOracleKiteMigration)
	if err != nil {
...
func (om oracleMigrator) commitMigration(c *infra.Container, data transactionData) error {
	...
	err := data.OracleTx.ExecContext(context.Background(), insertOracleKiteMigrationRow,
