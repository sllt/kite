...
func surrealSetup(t *testing.T) (migrator, *infra.MockSurrealDB, *infra.Container) {
	t.Helper()
...
func Test_SurrealGetLastMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		mockSurreal.EXPECT().Query(gomock.Any(), getLastSurrealDBKiteMigration, nil).Return([]any{
...
func Test_SurrealCommitMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		mockSurreal.EXPECT().Query(gomock.Any(), insertSurrealDBKiteMigrationRow, bindVars).Return([]any{}, tc.err)
