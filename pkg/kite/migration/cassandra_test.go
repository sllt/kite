...
func Test_CassandraGetLastMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		mockCassandra.EXPECT().QueryWithCtx(gomock.Any(), &lastMigration, getLastCassandraKiteMigration).Return(tc.err)
...
func Test_CassandraCommitMigration(t *testing.T) {
	...
	for i, tc := range testCases {
		mockCassandra.EXPECT().ExecWithCtx(gomock.Any(), insertCassandraKiteMigrationRow, td.MigrationNumber,
