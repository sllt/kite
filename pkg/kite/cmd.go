...
import (
	...
	cmd2 "github.com/sllt/kite/pkg/kite/cmd"
...
func (cmd *cmd) Run(c *infra.Container) {
...
func (cmd *cmd) addRoute(pattern string, handler Handler, options ...Options) {
	...
	for i := 0; i < len(pattern); i++ {
		if strings.ContainsRune(restrictedChars, rune(pattern[i])) {
			fmt.Println("found a restricted character in the command while registering with Kite:", pattern[i])
			return
		}
	}
