...
import (
	...
	"github.com/sllt/kite/pkg/kite/datasource/pubsub"
	"github.com/sllt/kite/pkg/kite/datasource/pubsub/google"
	"github.com/sllt/kite/pkg/kite/datasource/pubsub/kafka"
	"github.com/sllt/kite/pkg/kite/datasource/pubsub/mqtt"
	"github.com/sllt/kite/pkg/kite/datasource/redis"
	"github.com/sllt/kite/pkg/kite/datasource/sql"
	"github.com/sllt/kite/pkg/kite/logging"
	"github.com/sllt/kite/pkg/kite/logging/remotelogger"
	"github.com/sllt/kite/pkg/kite/metrics"
	"github.com/sllt/kite/pkg/kite/metrics/exporters"
	"github.com/sllt/kite/pkg/kite/service"
	"github.com/sllt/kite/pkg/kite/version"
...
func NewContainer(conf config.Config) *Container {
	...
	c := &Container{
		appName:    conf.GetOrDefault("APP_NAME", "kite-app"),
...
func (c *Container) Create(conf config.Config) {
	if c.appName == "" {
		c.appName = conf.GetOrDefault("APP_NAME", "kite-app")
	}
	...
	// Configure caller information display
	if cc, ok := c.Logger.(logging.CallerConfigurer); ok {
		showCaller := conf.GetOrDefault("LOG_CALLER", "true")
		cc.SetShowCaller(showCaller != "false")
	}
...
// HTTP services are registered from AddHTTPService method of Kite object.
...
func (c *Container) warnIfRedisPubSubSharesRedisDB(conf config.Config) {
	...
	// Kite migrations can later fail due to `kite_migrations` key-type collision (HASH vs STREAM).
	...
	if pubsubDB == redisDB {
		c.Logger.Warnf(
			"REDIS_PUBSUB_DB (%d) is the same as REDIS_DB (%d); migrations may fail (kite_migrations HASH/STREAM). "+
