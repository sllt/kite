...
func Test_newContextSuccess(t *testing.T) {
	...
	req := kiteHTTP.NewRequest(httpRequest)

	ctx := newContext(nil, req, infra.NewContainer(config.NewEnvFile("",
...
func TestContext_AddTrace(t *testing.T) {
	...
	tr := otel.GetTracerProvider().Tracer("kite-" + version.Framework)
...
func handleWebSocketMessage(ctx *Context, messageChan chan string) (any, error) {
	...
	select {
	case messageChan <- "Hello! Kite":
	default:
	}

	return "Hello! Kite", nil
}
...
func testWebSocketConnection(t *testing.T, wsURL string, messageChan chan string, handlerDone chan struct{}) {
	...
	assert.Equal(t, "Hello! Kite", string(message))
...
func TestGetAuthInfo_BasicAuth(t *testing.T) {
	...
	*req = *req.Clone(ctx)
	...
	kiteRq := kiteHTTP.NewRequest(req)
