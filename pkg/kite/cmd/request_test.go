...
func TestRequest_Bind(t *testing.T) {
	...
	r := NewRequest([]string{"command", "-Name=kite", "-Valid=true", "-Value=12", "-test", "--name=Kite", ""})

	assert.Equal(t, "kite", r.Param("Name"), "TEST Failed.\n Unable to read param from request")

	assert.Equal(t, "true", r.Param("test"), "TEST Failed.\n Unable to read param from request")

	assert.Equal(t, "12", r.PathParam("Value"), "TEST Failed.\n Unable to read param from request")

	assert.Equal(t, "Kite", r.PathParam("name"), "TEST Failed.\n Unable to read param from request")
	...
	if a.Name != "kite" || !a.Valid || a.Value != 12 {
