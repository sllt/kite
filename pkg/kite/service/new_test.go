...
func TestHTTPService_createAndSendRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{"with query params, body and header", map[string]any{"key": "value", "name": []string{"kite", "test"}},
			[]byte("{Test Body}"), map[string]string{"header1": "value1"}, "key=value&name=kite&name=test", "application/json"},
		{"with query params, body, header and content type", map[string]any{"key": "value", "name": []string{"kite", "test"}},
			[]byte("{Test Body}"), map[string]string{"header1": "value1", "content-type": "application/json"},
			"key=value&name=kite&name=test", "application/json"},
		{"with query params, body, header and content type xml", map[string]any{"key": "value", "name": []string{"kite", "test"}},
			[]byte("{Test Body}"), map[string]string{"header1": "value1", "content-type": "application/xml"},
			"key=value&name=kite&name=test", "application/xml"},
	...
	for i, tc := range tests {
		...
		service := &httpService{
			...
			url:            server.URL,
			Tracer:         trace.NewTracerProvider().Tracer("kite-http-client"),
...
func TestHTTPService_Get(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "/test-path", r.URL.Path)
		assert.Equal(t, "key=value&name=kite&name=test", r.URL.RawQuery)
	...
	resp, err := service.Get(t.Context(), "test-path",
		map[string]any{"key": "value", "name": []string{"kite", "test"}})
...
func TestHTTPService_Put(t *testing.T) {
	...
	resp, err := service.Put(t.Context(), "test-path",
		map[string]any{"key": "value", "name": []string{"kite", "test"}}, []byte("{Test Body}"))
...
func TestHTTPService_createAndSendRequestCreateRequestFailure(t *testing.T) {
	...
	resp, err := service.createAndSendRequest(ctx,
		"!@#$", "test-path", map[string]any{"key": "value", "name": []string{"kite", "test"}},
...
func TestHTTPService_createAndSendRequestServerError(t *testing.T) {
	...
	// when params value is of type []string then last value is sent in request
	resp, err := service.createAndSendRequest(ctx,
		http.MethodPost, "test-path", map[string]any{"key": "value", "name": []string{"kite", "test"}},
		[]byte("{Test Body}"), map[string]string{"header1": "value1"})
...
func newService(t *testing.T, server *httptest.Server) *httpService {
	...
	return &httpService{
		Client: http.DefaultClient,
		url:    server.URL,
		Tracer: tp.Tracer("kite-http-client"),
		Logger: logging.NewMockLogger(logging.INFO),
	}
}
