...
import (
	...
	kiteWebSocket "github.com/sllt/kite/pkg/kite/websocket"
)

var errConnection = errors.New("can't create connection")

func initializeWebSocketMocks(t *testing.T) (kiteWebSocket.MockUpgrader, *kiteWebSocket.Manager) {
	t.Helper()

	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()

	mockUpgrader := kiteWebSocket.NewMockUpgrader(mockCtrl)

	wsManager := kiteWebSocket.New()
	wsManager.WebSocketUpgrader = &kiteWebSocket.WSUpgrader{Upgrader: mockUpgrader}
...
func Test_WSConnectionCreate_Success(t *testing.T) {
	...
	mockConn := &kiteWebSocket.Connection{
		Conn: &websocket.Conn{},
	}
