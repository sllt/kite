...
func (c *Context) Trace(name string) trace.Span {
	tr := otel.GetTracerProvider().Tracer("kite-context")
	...
	// getting incomplete redis spans when viewing the trace using correlationID. If we remove assigning the ctx to Kite
...
func newContext(w Responder, r Request, c *infra.Container) *Context {
	return &Context{
		Context:       r.Context(),
		Request:       r,
		responder:     w,
...
func newCMDContext(w Responder, r Request, c *infra.Container, out terminal.Output) *Context {
	return &Context{
		Context:       r.Context(),
		responder:     w,
		Request:       r,
