...
func createTestContext(method, path, id string, body []byte, cont *infra.Container) *Context {
	...
	return newContext(kiteHTTP.NewResponder(httptest.NewRecorder(), method), kiteReq, cont)
}
	...
	tests := []struct {
	...
	}{
		{
			...
			resp: &entity{
				...
				constraints: map[string]kiteSql.FieldConstraints{"id": {AutoIncrement: true, NotNull: false},
		...
		},
		{
			...
			resp: &entity{
				...
				tableName:  "user",
				restPath:   "users",
				constraints: map[string]kiteSql.FieldConstraints{"id": {AutoIncrement: false, NotNull: false},
...
func Test_CreateHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			desc:          "success case",
			dialect:       "mysql",
			reqBody:       []byte(`{"id":1,"name":"kite","isEmployed":true}`),
			id:            1,
			mockErr:       nil,
	...
	for i, tc := range tests {
		t.Run(tc.dialect+" "+tc.desc, func(t *testing.T) {
			...
			mockMetrics := kiteSql.NewMockMetrics(ctrl)
			...
			if tc.expectedQuery != "" {
				mocks.SQL.ExpectExec(tc.expectedQuery).WithArgs(tc.id, "kite", true).WillReturnResult(mocks.SQL.NewResult(10, 1))
			}
	...
	for _, dc := range dialectCases {
		...
		for i, tc := range tests {
			t.Run(dc.dialect+" "+tc.desc, func(t *testing.T) {
				...
				db, mock, _ := kiteSql.NewSQLMocksWithConfig(t, &kiteSql.DBConfig{Dialect: dc.dialect})
				c.SQL = db
	...
	for _, dc := range dialectCases {
		tests := []testCase{
			...
			},
			{
				desc:         "error From DB",
				id:           "3",
				reqBody:      []byte(`{"id":3,"name":"kite","isEmployed":false}`),
		...
		for i, tc := range tests {
			t.Run(dc.dialect+" "+tc.desc, func(t *testing.T) {
				...
				mock.ExpectExec(dc.expectedQuery).WithArgs("kite", true, tc.id).
